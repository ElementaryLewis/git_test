...
const fetchApi = async () => {
    ...
    try {
        ...
        doc1.textContent = result.slice(11, -3).replace(/\\u201c/g, '"').replace(/\\u201d/g, '"').replace(/\\u2019/g, "'").replace(/\\u2014/g, "'").replace(/\\u00b2/g, '²');
        doc1.setAttribute("id", "joke-text");
        document.body.appendChild(doc1);

        let jokeTeller = new SpeechSynthesisUtterance();
        jokeTeller.text = result.slice(11, -3).replace(/\\u201c/g, '"').replace(/\\u201d/g, '"').replace(/\\u2019/g, "'").replace(/\\u2014/g, "'").replace(/\\u00b2/g, '²');
